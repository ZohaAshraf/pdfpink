...
const TOOLS = [
  ...
  { id:"word-pdf",  from:"docx", to:"PDF",  ext:"pdf",  icon:"📝", label:"Word to PDF",        accept:".doc,.docx",        color:"#db2777", light:"#fdf2f8", desc:"Convert Word documents to professional PDF files instantly." },
  { id:"excel-pdf", from:"xlsx", to:"PDF",  ext:"pdf",  icon:"📈", label:"Excel to PDF",       accept:".xls,.xlsx",        color:"#ec4899", light:"#fdf2f8", desc:"Export Excel sheets as perfectly formatted PDFs." },
  { id:"ppt-pdf",   from:"pptx", to:"PDF",  ext:"pdf",  icon:"🎨", label:"PowerPoint to PDF",  accept:".ppt,.pptx",        color:"#f472b6", light:"#fdf2f8", desc:"Save your presentations as universally readable PDFs." },
  { id:"img-pdf",   from:"jpg",  to:"PDF",  ext:"pdf",  icon:"🖼️", label:"Image to PDF",       accept:".jpg,.jpeg,.png",   color:"#be185d", light:"#fff0f6", desc:"Combine JPG, PNG images into a single polished PDF." },
  ...
  { id:"merge-pdf", from:"pdf",  to:"PDF",  ext:"pdf",  icon:"🗂️", label:"Merge PDFs",         accept:".pdf",              color:"#db2777", light:"#fff0f6", desc:"Combine multiple PDF files into one clean document." },
  ...
  { id:"ocr-pdf",   from:"jpg",  to:"PDF",  ext:"pdf",  icon:"🔍", label:"OCR PDF",            accept:".pdf,.jpg,.png",    color:"#f472b6", light:"#fdf2f8", desc:"Extract searchable text from scanned PDFs using AI OCR." },
];
...
const FAQS = [
  ...
  { q:"How accurate is the conversion?",           a:"For PDF to Office formats we achieve 97–99% layout accuracy. OCR achieves 99.2% text accuracy on clean scans." },
  { q:"Which browsers and devices are supported?", a:"All modern browsers: Chrome, Firefox, Safari, Edge. Fully supported on iOS and Android mobile browsers." },
];
...
function Modal({ tool, onClose }) {
  ...
  const pick = (f) => { if (f) { setFile(f); setPhase("idle"); setErr(""); } };
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://v2.convertapi.com/convert/${tool.from}/to/${tool.ext}?Secret=${CONVERTAPI_SECRET}&StoreFile=true`,
        { method: "POST", body: fd }
      );
      setProg(75);
      if (!res.ok) {
  ...
  const statusLabel =
    ...
    prog < 50 ? "Analyzing document…" :
    prog < 80 ? "Converting format…" :
    prog < 95 ? "Preparing download…" : "Done!";
    ...
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{
        ...
        <div style={{ padding: sm?"18px":"28px 32px" }}>
          ...
          {phase === "error" && (
            <div style={{background:"#fff1f2",border:"1px solid #fecdd3",borderRadius:14,padding:"24px 18px",marginBottom:18,textAlign:"center"}}>
              <div style={{fontSize:36,marginBottom:8}}>❌</div>
              <div style={{fontWeight:800,color:"#be185d",fontSize:15,marginBottom:6}}>Conversion Failed</div>
              <div style={{color:"#6b7280",fontSize:13,marginBottom:16}}>{errMsg}</div>
              <button onClick={()=>{setPhase("idle");setErr("");}} style={{background:tool.color,border:"none",borderRadius:10,padding:"10px 24px",color:"#fff",fontWeight:800,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>
                Try Again
              </button>
            </div>
          )}
          ...
          {phase === "done" && (
            <div style={{textAlign:"center",padding:"16px 0 12px"}}>
              ...
              <p style={{color:"#9ca3af",fontSize:13,margin:"0 0 20px"}}>Your <strong>.{tool.ext}</strong> file has been downloaded! ✅</p>
              <button onClick={()=>{setFile(null);setPhase("idle");setProg(0);}} style={{background:`linear-gradient(135deg,${tool.color},#f472b6)`,border:"none",color:"#fff",padding:"14px 32px",borderRadius:14,fontWeight:800,fontSize:15,cursor:"pointer",fontFamily:"inherit",boxShadow:`0 10px 28px ${tool.color}55`}}>
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Inter','Segoe UI',sans-serif",color:T.text,transition:"background 0.3s,color 0.3s"}}>
      ...
      <nav style={{background:T.nav,backdropFilter:"blur(20px)",borderBottom:`1px solid ${T.border}`,padding:md?"0 16px":"0 40px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:500}}>
        ...
        {!md && (
          <div style={{display:"flex",gap:28,alignItems:"center"}}>
            {["Tools","How It Works","Features","FAQ"].map(l=>(
              <a key={l} href={`#${l.toLowerCase().replace(/ /g,"-")}`} style={{color:T.sub,textDecoration:"none",fontSize:14,fontWeight:600}}>{l}</a>
            ))}
          </div>
        )}
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={()=>setDark(!dark)} style={{background:dark?"#3d0028":"#fff0f6",border:`1px solid ${T.border}`,borderRadius:99,padding:"7px 14px",cursor:"pointer",fontSize:13,color:T.sub,fontWeight:700,transition:"all 0.2s"}}>{dark?"☀️ Light":"🌙 Dark"}</button>
          {md && <button onClick={()=>setMobNav(!mobileNav)} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:24,color:T.sub,lineHeight:1}}>☰</button>}
        </div>
      </nav>
      ...
      {mobileNav && md && (
        <div style={{background:T.card,borderBottom:`1px solid ${T.border}`,padding:"10px 20px",display:"flex",flexDirection:"column",gap:0}}>
          {["Tools","How It Works","Features","FAQ"].map(l=>(
            <a key={l} href={`#${l.toLowerCase().replace(/ /g,"-")}`} onClick={()=>setMobNav(false)} style={{color:T.sub,textDecoration:"none",fontSize:15,fontWeight:700,padding:"12px 0",borderBottom:`1px solid ${T.border}`,display:"block"}}>{l}</a>
          ))}
        </div>
      )}
      ...
      <header style={{textAlign:"center",padding:md?"52px 16px 44px":"88px 24px 72px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"linear-gradient(135deg,rgba(190,24,93,0.1),rgba(244,114,182,0.1))",border:"1px solid rgba(190,24,93,0.25)",borderRadius:99,padding:"7px 18px",fontSize:11,fontWeight:800,color:"#be185d",marginBottom:24,letterSpacing:1.8,textTransform:"uppercase"}}>✦ Free · Fast · Secure · No Sign-Up Required</div>
          <h1 style={{fontSize:sm?"30px":md?"44px":"clamp(52px,7vw,78px)",fontFamily:"'Cormorant Garamond',serif",fontWeight:800,lineHeight:1.08,marginBottom:20,letterSpacing:-1}}>
            <span style={{display:"block",color:T.text}}>Convert Any File</span>
            <span style={{display:"block",background:"linear-gradient(135deg,#be185d 0%,#f472b6 50%,#be185d 100%)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"shimmer 3s linear infinite"}}>Instantly. Beautifully.</span>
          </h1>
          ...
          <div style={{position:"relative",maxWidth:460,margin:"0 auto 48px"}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search: PDF to Word, compress, OCR…" style={{width:"100%",padding:"14px 44px 14px 50px",borderRadius:99,border:`2px solid ${T.border}`,background:T.input,color:T.text,fontSize:14,outline:"none",boxShadow:"0 8px 32px rgba(190,24,93,0.09)",fontFamily:"inherit",transition:"border 0.2s,box-shadow 0.2s"}}/>
            {search && <button onClick={()=>setSearch("")} style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:18,color:"#be185d"}}>×</button>}
          </div>
      ...
      <section id="tools" style={{maxWidth:1160,margin:"0 auto",padding:md?"0 12px 72px":"0 32px 96px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:sm?"1fr 1fr":md?"1fr 1fr":"repeat(auto-fill,minmax(220px,1fr))",gap:md?12:18}}>
          {filtered.map((tool, i) => (
            <button key={tool.id} className="tc" onClick={()=>setTool(tool)} style={{background:T.card,border:`1.5px solid ${T.border}`,borderRadius:20,padding:sm?"16px 12px":md?"18px 14px":"24px 20px",cursor:"pointer",textAlign:"left",boxShadow:"0 2px 12px rgba(190,24,93,0.05)",animation:`fadeUp 0.4s ease ${i*0.04}s both`,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                <span style={{background:tool.light,color:tool.color,fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:99,border:`1px solid ${tool.color}33`}}>{tool.from.toUpperCase()}</span>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {FAQS.map((f,i)=>(
            <div key={i} style={{background:T.card,borderRadius:16,border:`1.5px solid ${faqOpen===i?"#be185d":T.border}`,overflow:"hidden",transition:"border 0.2s"}}>
              ...
              {faqOpen===i && (
                <div style={{padding:"0 20px 16px",color:T.sub,fontSize:13,lineHeight:1.7,borderTop:`1px solid ${T.border}`,paddingTop:14}}>{f.a}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section style={{padding:md?"0 12px 72px":"0 32px 96px"}}>
        <div style={{maxWidth:860,margin:"0 auto",borderRadius:28,background:"linear-gradient(135deg,#be185d 0%,#db2777 40%,#f472b6 100%)",padding:md?"44px 24px":"72px 56px",textAlign:"center",boxShadow:"0 32px 80px rgba(190,24,93,0.35)",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",zIndex:1}}>
            ...
            <h2 style={{margin:"0 0 14px",fontFamily:"'Cormorant Garamond',serif",fontSize:md?26:42,fontWeight:800,color:"#fff"}}>Ready to Convert Your Files?</h2>
            <p style={{color:"rgba(255,255,255,0.85)",fontSize:md?14:17,margin:"0 0 32px",lineHeight:1.7,maxWidth:480,marginLeft:"auto",marginRight:"auto"}}>No sign-up. No downloads. No limits on free conversions. Start instantly.</p>
            <button className="cta" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{background:"#fff",color:"#be185d",border:"none",padding:md?"13px 28px":"17px 48px",borderRadius:14,fontSize:md?14:17,fontWeight:900,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit",boxShadow:"0 8px 28px rgba(0,0,0,0.18)"}}>⚡ Start Converting for Free →</button>
          </div>
        </div>
      </section>
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:32,marginBottom:36}}>
            <div style={{maxWidth:300}}>
              ...
              <p style={{color:T.sub,fontSize:12,lineHeight:1.8}}>Professional file conversion for everyone. Fast, secure, and completely free.</p>
            </div>
            {!sm && (
              <div style={{display:"flex",gap:48,flexWrap:"wrap"}}>
                ...
                <div>
                  ...
                  {["About","Privacy Policy","Terms of Service","Contact Us","API Docs","Blog"].map(l=>(
                    <div key={l} style={{marginBottom:8}}><a href="/" style={{color:T.sub,fontSize:12,textDecoration:"none",fontWeight:500}}>{l}</a></div>
                  ))}
                </div>
              </div>
            )}
